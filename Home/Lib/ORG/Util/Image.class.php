...
class Image {
    ...
    // 中文验证码
    static function GBVerify2($string,$width=180, $height=50,$length=1, $type='png', $fontface='simhei.ttf', $verifyName='verify') {
        import('ORG.Util.String');
        $code = $string;
        //$width = ($length * 45) > $width ? $length * 45 : $width;
        $_SESSION[$verifyName] = md5($code);
        ...
        for ($i = 0; $i < $length; $i++) {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0, 120)); //这样保证随机出来的颜色较深。
            $codex = String::msubstr($code, $i, 1);
            imagettftext($im, mt_rand(16, 20), mt_rand(20, 60), 40 * $i + 20, mt_rand(30, 35), $fontcolor, $fontface, $codex);
        }
        Image::output($im, $type);
    }
