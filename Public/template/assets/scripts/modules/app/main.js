...
define(function(require, exports, module) {
    ...
    var App = (require("units/globalAudio"), require("modules/app/statistics"), function($item) {
        function jump() {
        ...
        !function() {
            ...
            theClass._$app.on("mousedown touchstart", function(e) {
                ...
                if (isStart && (activePage || isFirstTime) && ("mousemove" == e.type ? (moveDistanceX = e.pageX - startX, moveDistanceY = e.pageY - startY) : (moveDistanceX = e.touches[0].pageX - startX, moveDistanceY = e.touches[0].pageY - startY), Math.abs(moveDistanceY) > Math.abs(moveDistanceX))
                	...
                	if(currentPage.classList.contains("stop-next-page")){
                		//class 包含了endPage ，向前翻页时，重置_isDisableFlipNextPage 为true
                		theClass._isDisableFlipNextPage = !1;
                	}
                	...
                	if(currentPage.classList.contains("stop-next-page")){
                		//class 包含了endPage ，则不能向后翻页
                		theClass._isDisableFlipNextPage = !0;
                	}
            ...
            theClass._$pages.not(theClass._$pages.last()).append(guideHtml);//取得排除掉最后一页的其他pages ，并添加向上滑动按钮
        }), function() {
            ...
            image = "http://" + location.host + "/" + picUrl, theClass._$app.wx({
                title: title,
                con: content,
                img: image,
                handler: {
    ...
    App.prototype.getQueryString = function(name){ //公共方法 ，取得url包含的参数
